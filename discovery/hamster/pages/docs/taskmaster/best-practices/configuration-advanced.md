---
description: Advanced configuration options for Taskmaster. Learn how Hamster helps teams plan, build, and ship software faster with AI agents and clear execution...
title: Advanced Configuration | Hamster
...
## `.taskmaster/config.json` File (Recommended)
...
* **Environment Variables (`.env` file):** Use ONLY for sensitive secrets (API keys, tokens)
* **`.taskmaster/config.json`:** Use for all non-secret operational settings (model IDs, base URLs, project defaults, provider metadata, logging levels)
* **Exception:** `TASKMASTER_SKIP_AUTO_UPDATE` and CI detection remain environment-based for operational control
...
Taskmaster provides manual git integration through the `--from-branch` option:
...
## Example `.env` File (for API Keys Only)
...
# Optional Endpoint Overrides (for third-party APIs)
...
**Note:** For non-secret settings like `VERTEX_PROJECT_ID`, `VERTEX_LOCATION`, and `OLLAMA_BASE_URL`, prefer setting these in `.taskmaster/config.json` using the CLI tools rather than environment variables. Environment variables should be reserved for sensitive secrets and operational overrides.

## Troubleshooting

### Configuration Errors

* If Taskmaster reports errors about missing configuration or cannot find the config file, run `task-master models --setup` in your project root to create or repair the file.
